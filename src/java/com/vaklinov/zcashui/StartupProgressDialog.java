...
public class StartupProgressDialog extends JFrame {
    ...
    public StartupProgressDialog(ZCashClientCaller clientCaller)
    {
        ...
        URL iconUrl = this.getClass().getClassLoader().getResource("images/mnz-logo-large.png");
		...
		JLabel zcashWalletLabel = new JLabel(
		    ...
		    "Monaize<span style=\"font-style:italic;font-weight:bold;font-size:15px;vertical-align:super\">" +
...
//        }
        ...
        System.out.println("Splash: checking if mnzd is already running...");
        ...
        if (!shouldStartZCashd) {
            System.out.println("Splash: mnzd already running...");
            // What if started by hand but taking long to initialize???
//            doDispose();
//            return;
        } else
        {
        	System.out.println("Splash: mnzd will be started...");
        }
        ...
        while(true) {
            ...
            } catch (IOException e)
            {
            	if (iteration > 4)
            	{
                setProgressText("please wait - daemon takes longer to start..."+Integer.toString(iteration));
            		continue;
            	}
            }
        ...
        Runtime.getRuntime().addShutdownHook(new Thread() {
            public void run() {
                System.out.println("Stopping mnzd because we started it - now it is alive: " +
                ...
                try
                {
	                ...
	                while (!StartupProgressDialog.this.waitFor(daemonProcess, 3000))
	                {
	                	...
	                	System.out.println("Waiting for " + ((end - start) / 1000) + " seconds for mnzd to exit...");
	                ...
	                if (StartupProgressDialog.this.isAlive(daemonProcess)) {
	                    	System.out.println("mnzd is still alive although we tried to stop it. " +
	                                           "Hopefully it will stop later!");
	                        //System.out.println("zcashd is still alive, killing forcefully");
	                        //daemonProcess.destroyForcibly();
	                    } else
	                        System.out.println("mnzd shut down successfully");
                } catch (Exception bad) {
                    System.out.println("Couldn't stop mnzd!");
                    bad.printStackTrace();
                }
            }
        });
